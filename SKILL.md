...
3. **生成模式选择（硬门禁）** —— 真多智能体协同 / 单智能体角色模拟 / 已有题池仅验证；
   **不可默认单智能体替代多智能体**，除非用户明确选择该兜底模式
4. **反向题设置** —— 是否包含反向题、若包含则比例多少（**默认不含**；通过弹出选择题确认，见第 4.1 步）
5. **生成策略确认** —— 策略师产出生成计划后，展示给用户，确认后再执行出题
6. **完整题池确认** —— 一轮内容优化和复核后，展示完整候选题池的总量与维度配额；确认的是完整题池，不是预先生成的简版
7. **嵌入来源选择** —— OpenAI / Jina / Hugging Face / 本地模型 / 预计算 embedding / skip（见第 5 步）
...
- **in-silico ≠ 应答结构效度**：验证层建网于**题项文本嵌入的语义相似度**，不是被试应答的
  ...
  测量不变性与专家内容效度。
- **三重越界**：原论文仅在**英文 + 已知结构构念 + 零反向题**条件下验证 in-silico≈应答效度；
  本 Skill 常用于 **①中文 ②访谈扎根的探索性结构 ③（可选）反向题**——三者均超出论文实证包络，
  叠加时论文的对应关系**不可外推**，真实样本追验证是硬性必需。
...
**禁止行为**：不得因为方便而直接默认单智能体多流程；若用户选择真多智能体但当前 Codex 环境
没有可用子代理，应再次弹出选择题询问「改用单智能体角色模拟 / 暂停等待可用环境 / 改为已有题池验证」。
最终选择必须写入 `generation_session_log.md` 的“生成模式”字段。

具体编排见 `reference/multi-agent-generation.md`。

#### 4.1 ⛔ 反向题设置 + 策略师制定生成计划

**先决：⛔ 反向题设置**——在策略师出计划前，用弹出选择题询问用户：
- 是否包含反向题？**默认「不包含」**（与原论文一致；反向题在嵌入管线中有冗余/方法因子
  风险，见 `reference/methodological-boundaries.md` 第五节）。
- 若包含，比例多少？（如 4:1 / 3:1 / 自定义；作为第二个独立决策门处理）
- 结果写入 `construct_template.json` 的 `reverse_items` 字段，传给策略师/出题者/审查者。

按用户已选择的生成模式调用**策略师（Strategist）**：
- 输入：构念 JSON（含 `reverse_items` 设置）+ 语言/人群信息
- 输出：《题项生成计划》（每个维度的题项侧重、属性分布、**按用户确定的反向题设置安排正反比**、边界标记）
- ⛔ **展示给用户确认**，用户可修改计划
...
语义一致性/冗余，不是应答层面的结构效度。在**中文 / 探索性结构 / 含反向题**三重越界场景下
尤其不可外推；真实被试上的 EGA/CFA、信度、测量不变性与外部专家内容效度均不可省略。
