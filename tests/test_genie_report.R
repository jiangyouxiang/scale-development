...
source(file.path(skill_root, "scripts", "genie_report.R"), local = TRUE, encoding = "UTF-8")
...
writeLines('{"provider":"local","embedding_model":"BAAI/bge-m3","input_rows":4,"run_overall":true,"uva_cut_off":0.2,"reverse_items":{"include":false,"ratio":"0","policy":"default_positive_only"},"method_reference":{"citation":"Russell-Lasalandra, Christensen, & Golino (2026)","title":"Generative psychometrics via AI-GENIE: Automatic item generation and validation with network-integrated evaluation","journal":"Behavior Research Methods","doi":"10.3758/s13428-026-03082-1"},"skill_version":"0.1.0-rc2"}', manifest, useBytes = TRUE)
...
stopifnot(any(grepl(decode_unicode_escapes("<U+65B9><U+6CD5><U+539F><U+7406><U+4E0E><U+6587><U+732E><U+4F9D><U+636E>"), report, fixed = TRUE)))
stopifnot(any(grepl("Russell-Lasalandra", report, fixed = TRUE)))
stopifnot(any(grepl("10.3758/s13428-026-03082-1", report, fixed = TRUE)))
stopifnot(any(grepl("in-silico", report, fixed = TRUE)))
stopifnot(any(grepl("UVA", report, fixed = TRUE) & grepl("EGA", report, fixed = TRUE) & grepl("bootEGA", report, fixed = TRUE)))
...
reverse_manifest <- file.path(out, "manifest_reverse.json")
writeLines('{"provider":"local","embedding_model":"BAAI/bge-m3","input_rows":4,"run_overall":true,"uva_cut_off":0.2,"reverse_items":{"include":true,"ratio":"4:1","policy":"explicit_user_requested_high_risk"},"method_reference":{"citation":"Russell-Lasalandra, Christensen, & Golino (2026)","title":"Generative psychometrics via AI-GENIE: Automatic item generation and validation with network-integrated evaluation","journal":"Behavior Research Methods","doi":"10.3758/s13428-026-03082-1"},"skill_version":"0.1.0-rc2"}', reverse_manifest, useBytes = TRUE)
reverse_out <- file.path(out, "reverse_items")
dir.create(reverse_out, recursive = TRUE, showWarnings = FALSE)
ans_reverse <- generate_genie_report(raw, file.path(out, "input.csv"), reverse_out, reverse_manifest, character())
stopifnot(ans_reverse$core_complete)
reverse_report <- readLines(file.path(reverse_out, "genie_validation_report.md"), encoding = "UTF-8", warn = FALSE)
stopifnot(!any(grepl("<U\\+", reverse_report)))
stopifnot(any(grepl(decode_unicode_escapes("<U+53CD><U+5411><U+9898><U+9AD8><U+98CE><U+9669><U+63D0><U+793A>"), reverse_report, fixed = TRUE)))
stopifnot(any(grepl(decode_unicode_escapes("<U+9898><U+6C60><U+5305><U+542B><U+53CD><U+5411><U+9898>"), reverse_report, fixed = TRUE)))
